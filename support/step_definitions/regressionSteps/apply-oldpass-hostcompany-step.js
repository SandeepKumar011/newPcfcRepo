...
const username=oneDayData.regressionDataHost.approveUsername
const password=oneDayData.regressionDataHost.approvePassword
const actualEid=oneDayData.regressionDataHost.existingEid
...
Given('user navigates the login page for host', async ({page}) => {
  ...
  When('user enter the crendential for the host', async ({page}) => {
        const pageConstants = new PageConstants(page);
        await page.waitForLoadState("networkidle");
  ...
  Then('user should be redirected to the home page for host', async ({page}) => {
    const pageConstants = new PageConstants(page);
    await page.waitForLoadState("networkidle");
  ...
  When('user enter the infomation for pass for host', async ({page}) => {
               const pageConstants = new PageConstants(page);
                await page.waitForLoadState("networkidle");
                await page.waitForTimeout(2000);
  ...
  When('user enter infomation for the visitor for host', async ({page}) => {
    const pageConstants = new PageConstants(page);
    console.log('there is not need to enter visitor infomation');
  });
  
  When('user apply the pass for one day for host', async ({page}) => {
    const pageConstants = new PageConstants(page);
  ...
  Then('verify confirmation message for host', async ({page}) => {
            const pageConstants = new PageConstants(page);
            await page.waitForLoadState("networkidle");
