...
const portName=oneDayData.renewDataHost.portName
const gateType=oneDayData.renewDataHost.gateType
const passDuration=oneDayData.renewDataHost.passDuration
const passType=oneDayData.renewDataHost.passType
const reasonVisit=oneDayData.renewDataHost.visitReason
const hCompany=oneDayData.renewDataHost.hostCompany
const username=oneDayData.renewDataHost.username
const password=oneDayData.renewDataHost.password
const actualEid=oneDayData.renewDataHost.existingEid
...
Given('user navigates to the login page for renew host', async ({}) => {
  ...
  When('user enter the credential for the login for renew host', async ({}) => {
              const pageConstants = new PageConstants(page);
              await page.waitForLoadState("networkidle");
  ...
  Then('user redirected to the home page for renew host', async ({}) => {
    const pageConstants = new PageConstants(page);
    await page.waitForLoadState("networkidle");
  ...
  When('user search the pass for the renew pass host', async ({}) => {
    const pageConstants = new PageConstants(page);
    await page.waitForLoadState("networkidle");
    await page.waitForTimeout(5000);
    referceNumber=oneDayData.renewDataHost.expiredEid
    await pageConstants.passPage.searchForPassRefence.type(referceNumber);
  });
  
  When('user entere the infomration for renew pass host', async ({}) => {
    const pageConstants = new PageConstants(page);
       await page.waitForLoadState("networkidle");
  ...
  When('user submit the visitor information for the renew pass host', async ({}) => {
    const pageConstants = new PageConstants(page);
    await page.waitForLoadState("networkidle");
  ...
  When('user paid amount for the apply pass for the renew pass host', async ({}) => {
    console.log('for one day pass there is not need for the payment');
  });
  
  Then('verify successfully messge for the renew pass host', async ({}) => {
   const pageConstants = new PageConstants(page);
       await page.waitForLoadState("networkidle");
