...
const portName=oneDayData.renewDataContractor.portName
const gateType=oneDayData.renewDataContractor.gateType
const passDuration=oneDayData.renewDataContractor.passDuration
const passType=oneDayData.renewDataContractor.passType
const reasonVisit=oneDayData.renewDataContractor.visitReason
const hCompany=oneDayData.renewDataContractor.hostCompany
const username=oneDayData.renewDataContractor.username
const password=oneDayData.renewDataContractor.password
const actualEid=oneDayData.renewDataContractor.existingEid
...
Given('user navigates to the login page for renew contractor', async ({page}) => {
  ...
  When('user enter the credential for the login for renew contractor', async ({page}) => {
             const pageConstants = new PageConstants(page);
             await page.waitForLoadState("networkidle");
  ...
  Then('user redirected to the home page for renew contractor', async ({page}) => {
    const pageConstants = new PageConstants(page);
    await page.waitForLoadState("networkidle");
  ...
  When('user search the pass for the renew pass contractor', async ({page}) => {
    const pageConstants = new PageConstants(page);
     await page.waitForLoadState("networkidle");
     await page.waitForTimeout(5000);
     referceNumber=oneDayData.renewDataContractor.expiredEid
     await pageConstants.passPage.searchForPassRefence.type(referceNumber);
  });
  
  When('user entere the infomration for renew pass contractor', async ({page}) => {
    const pageConstants = new PageConstants(page);
       await page.waitForLoadState("networkidle");
  ...
  When('user submit the visitor information for the renew pass contractor', async ({page}) => {
    const pageConstants = new PageConstants(page);
    await page.waitForLoadState("networkidle");
  ...
  When('user paid amount for the apply pass for the renew pass contractor', async ({page}) => {
    console.log('for one day pass there is not need for the payment');
  });
  
  Then('verify successfully messge for the renew pass contractor', async ({page}) => {
      const pageConstants = new PageConstants(page);
      await page.waitForLoadState("networkidle");
