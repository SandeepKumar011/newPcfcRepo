...
const portName=oneDayData.sanityDataHost.portName
const gateType=oneDayData.sanityDataHost.gateType
const passDuration=oneDayData.sanityDataHost.passDuration
const passType=oneDayData.sanityDataHost.passType
const reasonVisit=oneDayData.sanityDataHost.visitReason
const hCompany=oneDayData.sanityDataHost.hostCompany
const visDesignation=faker.helpers.arrayElement(['Ac Technician', 'Account Assistant','Admin']);
const approvalusername=oneDayData.sanityDataHost.approveUsername
const approvalpassword=oneDayData.sanityDataHost.approvePassword
const hostusername=oneDayData.sanityDataHost.hostUsername
const hostpassword=oneDayData.sanityDataHost.hostPassword
...
Given('user navigates to the login page for host for one day', async ({page}) => {      
  ...
  Then('user redirected to the home page for host for one day', async ({page}) => {
    const pageConstants = new PageConstants(page);
    await page.waitForLoadState("networkidle");
  ...
  Then('user save the visitor on the add page for the host for one day', async ({page}) => {
    const pageConstants = new PageConstants(page);
  ...
  Then('verify user is added in the list for the host for one day', async ({page}) => {
    const pageConstants = new PageConstants(page);
    await expect(pageConstants.passPage.editButton).toBeVisible();
  });
  
  When('user submit the visitor information for the host for one day', async ({page}) => {
    const pageConstants = new PageConstants(page);
  ...
  When('user paid amount for the apply pass for the host for one day', async ({page}) => {
     console.log("there is not need to pay for one day pass")
  });
  
  Then('verify pass apply successfully messge for the host for one day', async ({page}) => {
        const pageConstants = new PageConstants(page);
        await page.waitForLoadState("networkidle");
...
When(/^user select logout button on home page for the host for one day$/, async({page}) => {
    const pageConstants = new PageConstants(page);
    await page.waitForTimeout(2000);
...
Then(/^verify pass approved successfully message for the host for one day$/, async({page}) => {
	const pageConstants = new PageConstants(page);
    await page.waitForLoadState("networkidle");
    await page.waitForTimeout(3000);
