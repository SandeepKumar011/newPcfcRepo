...
const twoMonthData=require('../../test_data/twoMonthData.json');
const { faker, fa } = require('@faker-js/faker');
const exp = require('constants');

const portName=twoMonthData.twoMonthDataHost.portName
const gateType=twoMonthData.twoMonthDataHost.gateType
const passDuration=twoMonthData.twoMonthDataHost.passDuration
const passType=twoMonthData.twoMonthDataHost.passType
const reasonVisit=twoMonthData.twoMonthDataHost.visitReason
const hCompany=twoMonthData.twoMonthDataHost.hostCompany
const visDesignation=faker.helpers.arrayElement(['Ac Technician', 'Account Assistant','Admin']);
const usernameLogin=twoMonthData.twoMonthDataHost.username
const passwordLogin=twoMonthData.twoMonthDataHost.password
const approvalusername=twoMonthData.twoMonthDataHost.approveUsername1
const approvalpassword=twoMonthData.twoMonthDataHost.approvePassword1
const approvalusername4=twoMonthData.twoMonthDataHost.approveUsername4
const approvalpassword4=twoMonthData.twoMonthDataHost.approvePassword4
...
Given('user navigates to the login page for company for two Month', async ({page}) => {
  ...
  When('user enter the credential for the login for company two Month', async ({page}) => {
       const pageConstants = new PageConstants(page);
       await page.waitForLoadState("networkidle");
  ...
  Then('user redirected to the home page for company for two Month', async ({page}) => {
    const pageConstants = new PageConstants(page);
    await page.waitForLoadState("networkidle");
  ...
  Then('Enter the pass information for the company for two month', async ({page}) => {
               const pageConstants = new PageConstants(page);
               await page.waitForLoadState("networkidle");
  ...
  Then('user enter the visitor information for the company for two month', async ({page}) => {
              const pageConstants = new PageConstants(page);
              await page.waitForTimeout(2000);
  ...
  Then('user save the visitor on the add page for the company for two month', async ({page}) => {
    const pageConstants = new PageConstants(page);
  ...
  Then('verify user is added in the list for the company for two month', async ({page}) => {
    const pageConstants = new PageConstants(page);
    await expect(pageConstants.passPage.editButton).toBeVisible();
  });
  
  When('user submit the visitor information for the company for two month', async ({page}) => {
    const pageConstants = new PageConstants(page);
  ...
  When('user paid amount for the apply pass for the company for two month', async ({page}) => {
    const pageConstants = new PageConstants(page);
              await page.waitForLoadState("networkidle");
              await page.waitForTimeout(10000);
  ...
  Then('verify pass apply successfully messge for the company for two month', async ({page}) => {
                  const pageConstants = new PageConstants(page);
                  await page.waitForLoadState("networkidle");
...
When(/^user select logout button on home page for the company for two month$/, async({page}) => {
    const pageConstants = new PageConstants(page);
    await page.waitForTimeout(2000);
...
Then(/^verify logout verification message for the company for two month$/, async({page}) => {
	const pageConstants = new PageConstants(page);
    await page.waitForLoadState("networkidle");
    await expect(pageConstants.loginPage.enterUsername).toBeVisible();
});

When(/^user approve the apply pass on the list page for the company for two month$/, async({page}) => {
	            const pageConstants = new PageConstants(page);
                await page.waitForLoadState("networkidle");
                await page.waitForTimeout(5000);
...
Then(/^verify pass approved successfully message for the company for two month$/, async({page}) => {
	                  const pageConstants = new PageConstants(page);
                      await page.waitForLoadState("networkidle");
                      await page.waitForTimeout(5000);
...
When(/^verify second second approval message for the company two month$/, async({page}) => {
	          console.log('there is no need for the second approval');
});

When(/^verify Third approval message for the company two month$/, async({page}) => {
    console.log('there is no need for the third approval');
});

Then(/^verify final status completed for the applyied pass for two month$/, async({page}) => {
	            const pageConstants = new PageConstants(page);
                await page.waitForTimeout(2000);
                await pageConstants.loginPage.logoutDrop.click();
                await pageConstants.loginPage.logoutButton.click();
